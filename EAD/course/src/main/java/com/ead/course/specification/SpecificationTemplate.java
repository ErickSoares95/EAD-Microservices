...
public class SpecificationTemplate {
    ...
    public static Specification<ModuleModel> moduleCourseId(final UUID courseId) {
        //root item buscado, query onde será realizada(CriteriaQuery a query), cb criterio de busca(criteriaBuilder)
        return (root, query, cb) -> {
            //Query não tem valores duplicados
            query.distinct(true);
            Root<ModuleModel> module = root;
            Root<CourseModel> course = query.from(CourseModel.class);
            //Busca os modulos em determiando curso
            Expression<Collection<ModuleModel>> coursesModules = course.get("modules");
            //Primeira parte verifica os course com determiando id. Segunda parte busca os modulos em determiando curso
